...
class EquiConv(torch.nn.Module):
    def __init__(self, num_subgroups, filter_sizes, filter_counts):
        ...
        out_type = nn.FieldType(self.r2_act, self.filter_counts[4]*[self.r2_act.regular_repr])
        self.block5 = self.make_block(in_type=in_type, out_type=out_type, filter_size=self.filter_sizes[4])
